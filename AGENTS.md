...
- `Brewfile` is the Homebrew dependency source. `bootstrap/setup/install_brew_deps.sh` installs the same core tools used by the configs.
- `bootstrap/setup/link_configs.sh` creates symlinks into `$HOME` and backs up existing targets with `.backup.<timestamp>` suffixes.

## Verification
- For shell, Vim, or terminal config changes, prefer `./bootstrap/checks/check_configs.sh`. It runs `bash -n` for repo scripts, loads `.vimrc` with Vim, and validates Ghostty config when `ghostty` is installed.
- For local environment/symlink health, use `./bootstrap/checks/doctor.sh`. It is environment-dependent and may fail because the user's home directory is not linked or dependencies are missing.
...
## Submodules
- After clone, initialize plugins/submodules with `./bootstrap/submodules/update_submodules.sh`.
- Intentionally update plugin pointers with `./bootstrap/submodules/update_submodules.sh --remote`; this changes tracked submodule commits.
...
- Do not run `bootstrap/setup/link_configs.sh` unless the task is specifically about installing/linking dotfiles; it mutates files in `$HOME`.
